/* ═══════════════════════════════════════════════
   Canvas Combat — UI 控制(導航 / 選角 / 結算 / 教師 / 圖鑑)
   ═══════════════════════════════════════════════ */

const UI = {
  sel: { mode: null, picks: [], aiLevel: 'normal', pickIndex: 0 },
  unlocks: JSON.parse(localStorage.getItem('cc_unlocks') || '{}'),
  board: JSON.parse(localStorage.getItem('cc_board') || '[]'),
  ...
  renderCharSelect() {
    ...
    const cardHTML = (c, i) => {
      ...
      <button class="char-card ${picked ? 'picked' : ''}" data-id="${c.id}" style="--h:${c.hue};--h2:${c.hue2};--d:${Math.min(i * 0.04, 0.6)}s">
        ...
        <span class="char-portrait">${c.img ? `<img src="${c.img}" alt="${c.name}" loading="lazy">` : `<span class="portrait-mono">${c.mono}</span>`}</span>
    ...
    det.innerHTML = `
      <div class="detail-inner" style="--h:${c.hue};--h2:${c.hue2}">
        <div class="detail-left">
          <div class="detail-portrait">${c.img ? `<img src="${c.img}" alt="${c.name}">` : `<span class="portrait-mono">${c.mono}</span>`}</div>
          <div class="detail-name">${c.name}</div>
          <div class="detail-title">${c.title} · ${c.origin}</div>
          <div class="detail-era">${c.era} · ${FACTIONS[c.medium] ? FACTIONS[c.medium].icon + ' ' : ''}${c.medium}</div>
  ...
  showResult(state, winnerIdx) {
    ...
    if (playerWon || isVersus) {
      ...
      if (!this.unlocks[uc] && uc !== 'boss-blank') {
        this.unlocks[uc] = true;
        localStorage.setItem('cc_unlocks', JSON.stringify(this.unlocks));
  ...
  renderTeacher() {
    ...
    wrap.innerHTML = `
      <div class="teacher-grid fade-up">
        <div class="glass-box">
          <h3>新增題目</h3>
          <label class="fld">題目<textarea id="tq-q" rows="2" placeholder="例:《星夜》的作者是誰?"></textarea></label>
          <div class="fld-row">
            <label class="fld">選項 A(正確答案)<input id="tq-a" placeholder="梵谷"></label>
            <label class="fld">選項 B<input id="tq-b" placeholder="莫內"></label>
          </div>
          <div class="fld-row">
            <label class="fld">選項 C<input id="tq-c" placeholder="高更"></label>
            <label class="fld">選項 D<input id="tq-d" placeholder="塞尚"></label>
          </div>
          <div class="fld-row">
            <label class="fld">難度
    ...
    $('csv-export').onclick = () => {
      ...
      this.downloadFile('CanvasCombat-自訂題庫.csv', '﻿' + lines.join('\n'));
      AudioEngine.play('confirm');
    };
    $('csv-template').onclick = () => {
      const tpl = [CSV_HEADER.join(','),
        '"《蒙娜麗莎》現藏於哪座博物館?","羅浮宮","大英博物館","烏菲茲美術館","普拉多美術館","A","1","博物館","達文西名作,現藏巴黎羅浮宮"'].join('\n');
  ...
  renderGallery() {
    ...
    const gCardHTML = (c, i) => {
      ...
      <div class="gallery-card fade-up ${un ? 'unlocked' : ''}" style="--h:${c.hue};--h2:${c.hue2};--d:${Math.min(i * 0.04, 0.6)}s" data-id="${c.id}">
        <div class="g-head">
          <span class="g-mono">${c.img ? `<img src="${c.img}" alt="${c.name}" loading="lazy">` : `<span class="g-mono-letter">${c.mono}</span>`}</span>
  ...
  showSettings() {
    ...
    `, card => {
      ...
      card.querySelector('#set-reset').onclick = () => {
        ...
        ['cc_unlocks', 'cc_board', 'cc_custom_q', 'cc_custom_only'].forEach(k => localStorage.removeItem(k));
