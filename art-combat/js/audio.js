/* ═══════════════════════════════════════════════
   Canvas Combat — Web Audio 音效引擎(零素材合成音)
   ═══════════════════════════════════════════════ */

const AudioEngine = (() => {
  let ctx = null;
  let master, sfxGain, bgmGain;
  let bgmOn = JSON.parse(localStorage.getItem('cc_bgm') ?? 'true');
  let sfxOn = JSON.parse(localStorage.getItem('cc_sfx') ?? 'true');
  let bgmTimer = null;

  /* BGM 音樂檔(載入失敗時退回合成琶音循環) */
  ...
  const CHORDS = [
    ...
    [164.81, 220.0, 261.63, 329.63],  // Am/E
  ];
  let bgmStep = 0;
  function bgmLoop() {
    if (!bgmOn || !ctx) return;
    ...
    for (let i = 0; i < 8; i++) {
      if (Math.random() < 0.78) {
        const n = chord[Math.floor(Math.random() * chord.length)] * (Math.random() < 0.22 ? 2 : 1);
        pluck(n, i * beat, 0.32 + Math.random() * 0.18);
      }
      if (i % 4 === 0) pluck(chord[0] / 2, i * beat, 0.34);
    }
    bgmStep++;
  ...
  function toggleBGM() {
    bgmOn = !bgmOn;
    localStorage.setItem('cc_bgm', JSON.stringify(bgmOn));
    if (bgmOn) {
  ...
  function toggleSFX() {
    sfxOn = !sfxOn;
    localStorage.setItem('cc_sfx', JSON.stringify(sfxOn));
